...
const ItemDetailsModal = ({ item, onClose, onUpdate }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 dark:bg-[#11100F] z-[100] flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-200">
      {/* Header */}
      <div className="h-16 shrink-0 bg-surface border-b border-gray-200 dark:border-gray-800 px-4 sm:px-6 flex justify-between items-center shadow-[0_1px_3px_rgba(0,0,0,0.02)] z-10">
         <div className="flex items-center space-x-2 sm:space-x-4 overflow-hidden">
           <button onClick={onClose} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors text-ink-500 hover:text-ink-900"><ChevronLeft size={20} /></button>
           <h2 className="text-sm sm:text-lg font-bold text-ink-900 dark:text-white tracking-tight truncate max-w-[120px] sm:max-w-md">{localItem.title || 'Untitled Video'}</h2>
           <span className="hidden sm:inline-block px-2.5 py-1 bg-gray-100 dark:bg-gray-800 text-[10px] uppercase tracking-wider font-bold text-ink-500 rounded">{localItem.status}</span>
         </div>
         <div className="flex items-center space-x-3 sm:space-x-4">
           <div className="flex items-center space-x-1.5 text-[11px] sm:text-xs font-medium text-ink-400">
             {saveStatus === 'Saving...' && <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 animate-pulse"></span>}
             {saveStatus === 'Saved' && <span className="w-1.5 h-1.5 rounded-full bg-green-400"></span>}
             <span className="hidden sm:inline">{saveStatus}</span>
           </div>
           <button onClick={handleSaveAndClose} className="btn-primary text-xs sm:text-sm px-4 py-2 rounded-xl bg-ink-900 hover:bg-ink-500 shadow-sm flex items-center space-x-2">
              <Save size={14} /> <span className="hidden sm:inline">Close Editor</span><span className="sm:hidden">Close</span>
           </button>
         </div>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          <div className="w-full md:w-[30%] md:min-w-[320px] md:max-w-[400px] bg-surface border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-800 overflow-y-auto custom-scrollbar p-6 md:p-8 shrink-0">
            <h3 className="text-[11px] font-bold text-ink-400 uppercase tracking-widest mb-6 pb-2 border-b border-gray-100 dark:border-gray-800">Project Strategy</h3>
            
            <div className="space-y-6">
              <div>
                <label className="block text-[11px] font-bold text-ink-900 dark:text-white uppercase tracking-wider mb-2">Video Title</label>
                <input type="text" value={localItem.title} onChange={e => setLocalItem({...localItem, title: e.target.value})} className="input-field bg-gray-50 border-gray-200 dark:border-gray-800 font-semibold" placeholder="Title..." />
              </div>

              <div>
                <label className="block text-[11px] font-bold text-ink-900 dark:text-white uppercase tracking-wider mb-2">Production Status</label>
                <select 
                  value={localItem.status} 
                  onChange={e => setLocalItem({...localItem, status: e.target.value})} 
                  className="input-field bg-gray-50 border-gray-200 dark:border-gray-800 font-medium"
                >
              ...
              <div>
                 <label className="block text-[11px] font-bold text-ink-900 dark:text-white uppercase tracking-wider mb-2">Hook (First 5 Seconds)</label>
                 <textarea rows="3" value={localItem.hook || ''} onChange={e => setLocalItem({...localItem, hook: e.target.value})} className="input-field bg-gray-50 border-gray-200 dark:border-gray-800 resize-none text-sm leading-relaxed" placeholder="Write a killer hook..." />
              </div>
              
              <div>
                 <label className="block text-[11px] font-bold text-ink-900 dark:text-white uppercase tracking-wider mb-2">Thumbnail Concept</label>
                 <textarea rows="2" value={localItem.thumbnail_text || ''} onChange={e => setLocalItem({...localItem, thumbnail_text: e.target.value})} className="input-field bg-gray-50 border-gray-200 dark:border-gray-800 resize-none text-sm leading-relaxed" placeholder="Visual idea..." />
              </div>
                ...
                <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="flex space-x-2 mb-3">
                    <input 
                      ...
                      className="input-field bg-gray-50 border-gray-200 dark:border-gray-800 flex-1 text-sm font-mono" 
                      placeholder="Video ID (e.g. dQw4w9WgXcQ)" 
                    />
                    <button 
                      onClick={handleSync} 
                      disabled={isSyncing}
                      className="bg-red-50 hover:bg-red-100 dark:bg-red-900/10 text-red-600 px-3 rounded-xl transition-colors disabled:opacity-50"
                  ...
                  {(localItem.kpi_views > 0 || localItem.kpi_ctr > 0) && (
                    <div className="grid grid-cols-3 gap-2 bg-gray-50 p-3 rounded-xl border border-gray-200 dark:border-gray-800">
                      <div>
                        <div className="text-[10px] text-ink-500 uppercase font-bold">Views</div>
                        <div className="text-sm font-bold text-ink-900 dark:text-white">{localItem.kpi_views.toLocaleString()}</div>
                      </div>
                      <div>
                        <div className="text-[10px] text-ink-500 uppercase font-bold">CTR</div>
                        <div className="text-sm font-bold text-ink-900 dark:text-white">{localItem.kpi_ctr}%</div>
                      </div>
                      <div>
                        <div className="text-[10px] text-ink-500 uppercase font-bold">Retention</div>
                        <div className="text-sm font-bold text-ink-900 dark:text-white">{localItem.kpi_retention}%</div>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
                 <div className="flex justify-between items-center mb-4">
                   <label className="block text-[11px] font-bold text-ink-900 dark:text-white uppercase tracking-wider">Project Files</label>
                   <label className="text-[10px] font-bold uppercase tracking-wider px-2 py-1 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-ink-500 rounded cursor-pointer transition-colors flex items-center space-x-1">
                 ...
                 <div className="space-y-2">
                    {attachments.length === 0 ? (
                      <div className="p-4 border border-dashed border-gray-200 dark:border-gray-800 rounded-xl bg-gray-50 dark:bg-gray-900/50 text-center text-xs text-ink-400 font-medium">
                        No files attached.
                      </div>
                    ) : (
                      attachments.map((path, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex justify-between items-center p-2.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl group hover:border-ink-300 transition-colors">
                            <button onClick={() => openAttachment(path)} className="flex items-center space-x-3 text-xs text-ink-900 dark:text-white font-medium flex-1 text-left truncate">
                              <div className="p-1 bg-surface border border-gray-200 dark:border-gray-800 rounded-md shrink-0 text-ink-500"><Paperclip size={12} /></div>
                              <span className="truncate hover:underline">{filename}</span>
                            </button>
                            <button onClick={() => removeAttachment(path)} className="text-gray-400 hover:text-red-500 md:opacity-0 md:group-hover:opacity-100 p-1 transition-all shrink-0 ml-2">
                              <X size={14} />
                            </button>
                          </div>
                        )
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-gray-50/50 dark:bg-gray-900/10 pb-20">
             <div className="max-w-4xl mx-auto py-6 md:py-10 px-4 md:pl-16 md:pr-8">
                <div className="flex justify-between items-center mb-8 border-b border-gray-200 dark:border-gray-850 pb-4 sticky top-0 bg-gray-50/90 dark:bg-[#11100F]/90 backdrop-blur-md z-10 -mt-6 md:-mt-10 pt-6 md:pt-10">
                  <h2 className="text-md md:text-xl font-bold text-ink-900 dark:text-white tracking-tight flex items-center">
                    <FileText size={18} className="mr-2 text-ink-500" /> Script Editor
                    <button onClick={handlePrint} className="ml-3 flex items-center space-x-1.5 text-[10px] sm:text-xs font-bold bg-surface border border-gray-200 dark:border-gray-800 px-2.5 py-1.5 rounded-lg text-ink-500 hover:text-ink-900 hover:border-ink-300 transition-colors shadow-sm no-print">
                      <Printer size={12} /> <span>Print</span>
                    </button>
                  </h2>
                  <div className="hidden md:flex items-center space-x-4 w-2/3">
                ...
                <div className="space-y-6">
                  {blocks.map((block, index) => (
                    <div key={block.id} className="group relative flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-4 items-start border-b border-gray-200 md:border-b-0 pb-6 md:pb-0">
                      {/* Line Controls */}
                      <div className="flex md:absolute md:-left-10 top-0 flex-row md:flex-col items-center bg-surface border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm overflow-hidden py-0.5 md:py-1 z-10 shrink-0 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                         <span className="text-[10px] font-bold text-gray-400 px-3 md:px-2 md:pb-1 border-r md:border-r-0 md:border-b border-gray-100 dark:border-gray-800 leading-none md:leading-normal">{index + 1}</span>
                         <button onClick={() => addBlock(index)} className="p-2 md:p-1.5 text-gray-400 hover:text-ink-900 hover:bg-gray-50 border-r md:border-r-0 border-gray-100 dark:border-gray-850" title="Add block below"><Plus size={14} /></button>
                         <button onClick={() => removeBlock(block.id)} className="p-2 md:p-1.5 text-gray-400 hover:text-red-500 hover:bg-gray-50" title="Remove block"><X size={14} /></button>
                      </div>
                      
                      <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                         <div className="relative">
                           <div className="md:hidden text-[10px] font-bold text-ink-500 uppercase tracking-widest mb-1.5 pl-1.5 border-l-2 border-ink-900 dark:border-white">A-Roll (Audio / Speech)</div>
                           <textarea 
                             value={block.a_roll} 
                             onChange={(e) => updateBlock(block.id, 'a_roll', e.target.value)}
                             placeholder="What you'll say..."
                             className="w-full bg-surface border border-gray-200 dark:border-gray-800 rounded-xl p-4 md:p-5 text-[14px] md:text-[15px] text-ink-900 dark:text-white focus:ring-2 focus:ring-ink-900 focus:border-transparent outline-none min-h-[90px] md:min-h-[120px] leading-relaxed shadow-sm transition-shadow hover:shadow-md resize-y"
                           />
                         </div>
                         <div className="relative">
                           <div className="md:hidden text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-1.5 pl-1.5 border-l-2 border-blue-500">B-Roll (Visuals / SFX)</div>
                           <textarea 
                             value={block.b_roll} 
                             onChange={(e) => updateBlock(block.id, 'b_roll', e.target.value)}
                             placeholder="What the viewer sees (b-roll, text, SFX)..."
                             className="w-full bg-blue-50/20 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/35 rounded-xl p-4 md:p-5 text-[13px] md:text-[14px] text-ink-700 dark:text-blue-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none min-h-[90px] md:min-h-[120px] leading-relaxed shadow-sm transition-shadow hover:shadow-md resize-y"
                           />
                         </div>
                      </div>
                    </div>
                  ...
                  <div className="pt-6">
                    <button onClick={() => addBlock(blocks.length - 1)} className="w-full py-4 border-2 border-dashed border-gray-300 dark:border-gray-800 rounded-xl text-ink-400 font-bold text-sm uppercase tracking-wider hover:border-ink-900 hover:text-ink-900 transition-colors flex justify-center items-center hover:bg-gray-50 dark:hover:bg-gray-900/40">
